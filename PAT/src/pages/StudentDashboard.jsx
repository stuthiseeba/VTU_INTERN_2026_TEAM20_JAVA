...
function AppliedDrives({ userId, userEmail }) {
  const [applications, setApplications] = useState(
    JSON.parse(localStorage.getItem(`applied_drives_${userId}`) || '[]')
  );
  ...
  async function deleteApplication(app) {
    ...
    // Remove from backend
    try {
      const res = await fetch(`http://localhost:8080/api/tpo/drive/${app.driveId}/applicants`);
      const applicants = await res.json();
      const match = applicants.find(a => a.studentEmail === userEmail);
      if (match) {
        await fetch(`http://localhost:8080/api/tpo/drive/applicant/${match.id}`, { method: 'DELETE' }).catch(() => {});
      }
    } catch {}
    // Remove from localStorage
    const updated = applications.filter(a => a.driveId !== app.driveId);
    localStorage.setItem(`applied_drives_${userId}`, JSON.stringify(updated));
          ...
          <div className="content-item" key={i} style={{ flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%' }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: '#aaa', marginTop: 4 }}>Applied on: {a.appliedOn}</p>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
                <span className="badge badge-verified">{a.status}</span>
...
function AvailableDrives({ userId, userName, userEmail }) {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/tpo/drives")
      .then(r => r.json())
    ...
    fetch(`http://localhost:8080/api/student/profile/${userId}`)
      ...
      .then(d => {
        setAppData({
          ...
          phone: d.phone||'', linkedin: d.linkedin||'', address: d.address||'',
          gradYear: d.gradYear||'', cgpa: d.cgpa||'', department: d.department||'',
          college: d.college||'', degreeName: d.degreeName||'', specialization: d.specialization||'',
          yearDegree: d.yearDegree||'', softSkills: d.softSkills||'', techSkills: d.techSkills||''
        });
        setApplying(drive);
      })
      .catch(() => {
        setAppData({ name: userName, email: userEmail, phone:'', linkedin:'', address:'', gradYear:'', cgpa:'', department:'', college:'', degreeName:'', specialization:'', yearDegree:'', softSkills:'', techSkills:'' });
        setApplying(drive);
      });
  }
  ...
  async function submitApplication() {
    ...
    if (alreadyApplied) {
      alert(`You have already applied to ${applying.company}!`);
      return;
    }
    // Save to backend so TPO can see it
    try {
      await fetch("http://localhost:8080/api/tpo/drive/apply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          driveId: applying.id.toString(),
  ...
  return (
    <div className="dash-layout">
      ...
      <div className="dash-main">
        ...
        {tab === 'drives' && (
          <AppliedDrives userId={user.userId} userEmail={user.email} />
        )}
